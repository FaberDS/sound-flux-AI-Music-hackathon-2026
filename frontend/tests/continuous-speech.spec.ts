...
async function speakThenPause(page: Page) {
  await page.evaluate(() => {
    ;(
      window as unknown as { speechTest: { amplitude: number } }
    ).speechTest.amplitude = 0.06
  })
  await page.waitForTimeout(350)
  await page.evaluate(() => {
    ;(
      window as unknown as { speechTest: { amplitude: number } }
    ).speechTest.amplitude = 0
  })
}

for (const readAloud of [false, true]) {
  test(`sends after silence, resumes after the ${readAloud ? 'spoken' : 'text'} reply, and stops every microphone track`, async ({
    page,
  }) => {
    await prepare(page)
    let uploads = 0
    await page.route('**/api/v1/transcriptions', (route) => {
      uploads++
      return route.fulfill({ json: { text: 'Ich mag Musik.' } })
    })
    ...
    await page.route('**/api/v1/speech', (route) => {
      // A real, two-second WAV lets the browser's playback-end event resume listening.
      const audioBytes = 8_000 * 2 * 2
    ...
    await expect(
      page.getByRole('img', { name: 'Sound Flux: Ich höre zu' }),
    ).toBeVisible()
    ...
    if (readAloud) {
      await expect(
        page.getByRole('img', { name: 'Sound Flux: Ich spreche' }),
      ).toBeVisible()
      const capture = await page.evaluate(() => {
        const streams = (
          window as unknown as { speechTest: { streams: MediaStream[] } }
        ).speechTest.streams
        return {
      ...
      expect(capture).toEqual({ count: 1, stopped: true })
    }
    await expect
      .poll(() =>
        page.evaluate(
    ...
    expect(
      await page.evaluate(() =>
        (
          window as unknown as { speechTest: { streams: MediaStream[] } }
        ).speechTest.streams.every((stream) =>
          stream.getTracks().every((track) => track.readyState === 'ended'),
        ),
      ),
    ).toBe(true)
...
test('transcription failures end automatic listening instead of starting a retry loop', async ({
  page,
}) => {
  await prepare(page)
  await page.route('**/api/v1/transcriptions', (route) =>
  ...
  await expect(page.getByRole('alert')).toContainText(
    'Die Sprachbegleitung ist gerade nicht erreichbar',
  )
  await expect(
