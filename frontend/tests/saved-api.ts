...
import type {
  ...
  ProfileValues,
  SavedTurn,
} from '../src/lib/savedData'

export async function mockSavedApi(
  page: Page,
  ...
  const profile = (): ProfileState => ({
    ...
    onboarding: (() => {
      ...
      ).find((key) => !state.values[key])
      return key
        ? { key, label: key, question: key, category: 'Personal' }
        : null
    })(),
  })
  ...
  await page.route('**/api/v1/data', (route) => {
    state.values = {}
    state.history = []
    state.greeting = 'Welcome today.'
    state.deletions.push('all')
    return route.fulfill({ json: profile() })
  })
  return state
}
